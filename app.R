########################################################################
# APPLICATION SHINY - NeuroVigil - CM6RI BRANDING + TRILINGUAL
########################################################################
...
if (file.exists(favicon_path)) {
  FAVICON_URL <- "www/CM6RI-Picto32.png"
} else {
...
server <- function(input, output, session) {
  ...
  ## ---------------- PAGE : PRÉDICTION MANUELLE (3 COLONNES HORIZONTALES) ----------------
  page_prediction <- function() {
    div(class = "container-fluid", style = "padding: 28px;",
        h2(class = "titre-section", t("form_title"), style = "margin-bottom: 10px;"),
        p(em(t("form_hint")), style = paste0("color:", COL_TEXTE_ATTEN, "; font-size: 15px; line-height: 1.5; margin-bottom: 25px;")),
        
        # 3 Horizontal Columns for Patient Data
        fluidRow(
          # Column 1: Basic Info
          column(4,
                 div(class = "card-premium", style = "height: 100%;",
                     h4(class = "sous-titre", tags$span(class = "section-icon", "👤"), "Informations de base"),
          ...
          column(4,
                 div(class = "card-premium", style = "height: 100%;",
                     ...
                     p(style = paste0("font-size: 13px; color:", COL_TEXTE_ATTEN, "; margin-top: -6px; margin-bottom: 10px;"), t("field_optionnel")),
        ...
        div(style = "text-align: center; margin: 30px 0;",
            actionButton("predict_btn", t("btn_predict"), class = "btn-premium", style = "width: 320px; font-size: 16px; padding: 12px 24px;")
        ),
  ...
  output$resultats <- renderUI({
    if (is.null(valeurs$derniere_prediction)) {
      return(div(style = "text-align: center; padding: 60px 20px; color: #636E72; background: #F8F9FA; border-radius: 12px; border: 2px dashed #DEE2E6;",
                 ...
                 p(style = "font-size: 16px;", "Veuillez remplir les informations du patient dans les 3 colonnes ci-dessus et cliquer sur le bouton d'analyse pour afficher les résultats ici.")))
    }
